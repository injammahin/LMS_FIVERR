@if(!empty($note))
    <div x-data="{ open: false }"
        class="fixed left-4 bottom-4 sm:left-6 sm:bottom-6 z-[9999]"
        style="position: fixed; left: 24px; bottom: 24px;">

        <!-- Floating Notebook Button -->
        <button @click="open = true"
            class="inline-flex items-center gap-2 px-5 py-3 rounded-2xl bg-indigo-600 text-white shadow-xl hover:bg-indigo-700 transition-all duration-200">
        ...
        <div x-show="open"
            x-transition
            class="fixed inset-y-0 left-0 w-full max-w-2xl bg-white dark:bg-slate-900 notebook-drawer border-r border-gray-200 dark:border-white/10 z-[10000]"
            style="display:none;">
