...
@php
    ...
    $pendingReviewCalc = (int) $questions->whereIn('type', ['text','file'])->filter(function ($q) use ($answers) {
        $a = $answers->get($q->id);
        $payload = $a?->answer_json ?? [];
        return !empty($payload) || !empty($a?->file_path);
    })->count();

    // If controller already sends these, keep them; otherwise use calc
    $objectiveTotal = $objectiveTotal ?? $objectiveTotalCalc;
    $objectiveScore = $objectiveScore ?? $objectiveScoreCalc;
    $pendingReview  = $pendingReview  ?? $pendingReviewCalc;
@endphp
...
<style>
    ...
    .quizQuestionBody ul,.quizQuestionBody ol{ padding-left: 1.25rem; margin: .5rem 0; }
                ...
                <div class="bg-white rounded-3xl border border-gray-200 shadow-sm p-6">
                    <div class="flex items-start justify-between gap-4">
                        ...
                        <div class="text-right shrink-0">
                            ...
                            <span class="inline-flex mt-2 px-3 py-1 rounded-full border text-xs font-semibold {{ $statusClass }}">
                                {{ $statusLabel }}
                            </span>
                            ...
                            @if($answered)
                                <div class="text-xs text-gray-500 mt-2">
                                    Awarded:
                                    <span class="font-semibold">{{ (int)($ans?->awarded_marks ?? 0) }}</span>
                                </div>
                            @endif
                        </div>
                    </div>
