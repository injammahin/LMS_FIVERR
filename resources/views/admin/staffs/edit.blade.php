...
@section('content')
    ...
    <div class="max-w-2xl mx-auto space-y-6 text-xxs"
        x-data="{ loginType: '{{ $staff->username ? 'username' : 'email' }}', show: false }">

        <h1 class="text-lg font-semibold text-gray-800">Edit Staff</h1>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <form method="POST" action="{{ route('admin.staffs.update', $staff->id) }}" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div x-show="loginType==='email'" x-cloak>
                    ...
                    <input type="email" name="email" value="{{ old('email', $staff->email) }}"
                        :disabled="loginType!=='email'" class="w-full px-3 py-2 border rounded-lg">
                ...
                <div x-show="loginType==='username'" x-cloak>
                    ...
                    <input type="text" name="username" value="{{ old('username', $staff->username) }}"
                        :disabled="loginType!=='username'" class="w-full px-3 py-2 border rounded-lg">
