...
<div class="space-y-6"
    ...
    <div class="flex items-start justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-white/60">
                Staff: <span class="font-medium">{{ $staff->name }}</span>
                <span class="text-xs text-gray-400">({{ $staff->email ?? $staff->username }})</span>
            </p>
        </div>
    ...
    {{-- Assign Form --}}
    <form method="POST" action="{{ route('admin.staffs.courses.update', $staff->id) }}">
        @csrf
