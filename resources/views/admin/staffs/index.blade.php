...
@section('content')
    <div class="space-y-6 text-xxs" x-data="{
        ...
        <div class="flex justify-between items-center">
            <h1 class="text-lg font-semibold text-gray-800">All Staff</h1>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
            <div class="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-4">
                <div class="w-10 h-10 rounded-lg bg-blue-100 flex items-center justify-center text-blue-600">
                    <i class="fa-solid fa-user-gear text-xxs"></i>
                </div>
                <div>
                    <p class="text-xxs text-gray-500">Total Staff</p>
                    <p class="font-semibold text-gray-800">{{ $staffs->total() }}</p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="font-semibold text-gray-800">{{ $staffs->count() }}</p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="font-semibold text-gray-800">
                        {{ optional($staffs->first())->created_at?->format('d M') ?? '-' }}
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between p-4 border-b border-gray-200">
                <div>
                    <h2 class="text-xxs font-medium text-gray-700">Staff List</h2>
                    <p class="text-xxs text-gray-400">Support staff (no grading)</p>
                </div>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($staffs as $staff)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4">
                                    @if($staff->is_active)
                                        <span class="px-2 py-1 rounded text-[10px] bg-green-100 text-green-700">
                                            Active
                                        </span>
                                    @else
                                        <span class="px-2 py-1 rounded text-[10px] bg-red-100 text-red-700">
                                            Suspended
                                        </span>
                                    @endif
                                </td>
                                ...
                                <td class="px-6 py-4 text-right space-x-2">
                                    ...
                                    <a href="{{ route('admin.staffs.courses.edit', $staff->id) }}"
                                        class="inline-flex items-center justify-center px-3 py-1.5 rounded-lg border border-gray-200 hover:bg-gray-100 text-xxs">
                                        Assign Courses
                                    </a>
                                    ...
                                    <button type="button" @click="
                                                        openToggle = true;
                                                        toggleUrl = '{{ route('admin.staffs.toggle-status', $staff->id) }}';
                                                        toggleAction = '{{ $staff->is_active ? 'Suspend' : 'Activate' }}';
                                                        toggleName = '{{ addslashes($staff->name) }}';
                                                    "
                                        class="inline-flex items-center justify-center px-3 py-1.5 rounded-lg border border-gray-200 text-[10px]
                                                           {{ $staff->is_active ? 'bg-yellow-100 text-yellow-700 hover:bg-yellow-200' : 'bg-green-100 text-green-700 hover:bg-green-200' }}">
                                    ...
                                    <button type="button"
                                        @click="openDelete = true; deleteUrl = '{{ route('admin.staffs.destroy', $staff->id) }}'"
            ...
            <div class="flex justify-between items-center p-4 border-t border-gray-200 text-xxs text-gray-500">
                <span>
                    Showing {{ $staffs->firstItem() ?? 0 }}
                    to {{ $staffs->lastItem() ?? 0 }}
                    of {{ $staffs->total() }}
                </span>
        ...
        {{-- TOGGLE CONFIRMATION MODAL (Suspend/Activate) --}}
            ...
            <div @click.away="openToggle = false" class="bg-white rounded-xl shadow-lg w-96 p-6">
                ...
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <h3 class="font-semibold text-gray-800 text-sm">
                        <span x-text="toggleAction"></span> Staff
                    </h3>
                </div>
        ...
        <div x-show="openDelete" x-transition class="fixed inset-0 flex items-center justify-center bg-black/40 z-50">
            <div @click.away="openDelete = false" class="bg-white rounded-xl shadow-lg w-80 p-6">
                ...
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <h3 class="font-semibold text-gray-800 text-sm">Delete Staff</h3>
                </div>
