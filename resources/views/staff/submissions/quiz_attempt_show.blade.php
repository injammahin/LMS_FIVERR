        ...
        <div class="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
            ...
            <div class="p-5 flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                ...
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <span
                        class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold border {{ $isGraded ? $pill('graded') : $pill('submitted') }}">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div class="rounded-2xl border border-gray-200 bg-white shadow-sm p-5">
                <div class="flex items-start justify-between gap-3">
                    ...
                    <div
                        class="w-11 h-11 rounded-2xl bg-blue-50 border border-blue-100 text-blue-700 grid place-items-center">
                ...
                <div class="mt-4 text-xs text-gray-600 space-y-1">
                    <div>Started: <span
                            class="font-semibold text-gray-900">{{ optional($attempt->started_at)->format('d M Y, h:i A') ?? '—' }}</span>
                    </div>
                    <div>Submitted: <span
                            class="font-semibold text-gray-900">{{ optional($attempt->submitted_at)->format('d M Y, h:i A') ?? '—' }}</span>
                    </div>
                </div>
            </div>
                ...
                <div class="flex items-start justify-between gap-3">
                    <div>
                        ...
                        <div class="mt-2">
                            @if($isGraded && $total > 0)
                                <span
                                    class="inline-flex text-xs px-2 py-1 rounded-full border {{ $passed ? $pill('ok') : $pill('bad') }}">
                    ...
                    <div
                        class="w-11 h-11 rounded-2xl bg-emerald-50 border border-emerald-100 text-emerald-700 grid place-items-center">
        ...
        <form method="POST" action="{{ route('teacher.quiz.attempts.grade', $attempt->id) }}" class="space-y-5">
            ...
            <div class="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                <div class="p-5 border-b border-gray-200 flex items-center justify-between gap-3">
                    ...
                    <button
                        class="inline-flex items-center gap-2 px-5 py-2 rounded-xl bg-emerald-600 text-white hover:bg-emerald-700 text-sm font-semibold">
                        ...
                        <div class="p-6 space-y-4">
                            ...
                            <div class="flex items-start justify-between gap-4">
                                    ...
                                    @if($qImg)
                                        <div class="mt-3">
                                            <img src="{{ asset('storage/' . $qImg) }}"
                                                class="max-h-72 rounded-2xl border border-gray-200 shadow-sm" alt="Question image">
                                        </div>
                                    @endif
                                </div>
                                ...
                                <div class="shrink-0 text-right">
                                    ...
                                    <div class="mt-2">
                                        <span
                                            class="text-xs px-2 py-1 rounded-full border {{ $isManual ? $pill('manual') : $pill('auto') }}">
                            ...
                            <div class="rounded-2xl border border-gray-200 bg-gray-50 p-5 space-y-2">
                                ...
                                @if($type === 'true_false')
                                    <div class="text-sm text-gray-800">Answer: <span
                                            class="font-semibold">{{ $studentTF ?? '—' }}</span></div>
                                ...
                                @if(!$isManual)
                                    <div class="pt-3 border-t border-gray-200 flex flex-wrap items-center gap-2">
                                        <span
                                            class="text-xs px-2 py-1 rounded-full border {{ $ans->is_correct ? $pill('ok') : $pill('bad') }}">
                                            {{ $ans->is_correct ? 'Correct' : 'Incorrect' }}
                                        </span>

                                        <span class="text-xs px-2 py-1 rounded-full border bg-white text-gray-800 border-gray-200">
                                            Current: <span class="font-semibold">{{ (int) ($ans->awarded_marks ?? 0) }}</span> /
                                            {{ $marks }}
                                        </span>
                                    </div>
                                @endif
            ...
            <div class="flex justify-end">
                <button
                    class="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-emerald-600 text-white hover:bg-emerald-700 text-sm font-semibold">
                    <i class="fa-solid fa-floppy-disk"></i> Save Grade
                </button>
            </div>
        </form>
