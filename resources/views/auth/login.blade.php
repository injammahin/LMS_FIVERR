...
@section('content')
    ...
    <div class="min-h-screen flex items-center justify-center px-4 py-10 relative overflow-hidden bg-slate-950">
        ...
        <div class="relative w-full max-w-md">
            ...
            <div class="relative rounded-3xl border border-white/15 bg-white/10 backdrop-blur-2xl shadow-[0_20px_70px_rgba(0,0,0,.45)] overflow-hidden">
                {{-- top shine --}}
                ...
                <div class="p-7 sm:p-8">
                    ...
                    <div class="flex items-center gap-3">
                        <div class="h-11 w-11 rounded-2xl grid place-items-center text-white shadow-lg"
                             id="logoChip">
                            <span class="font-black text-lg">L</span>
                        </div>

                        <div class="min-w-0">
                            <h2 class="text-xl font-semibold text-white tracking-tight">Welcome back</h2>
                            <p class="text-sm text-white/70">Log in to continue to your portal</p>
                        </div>
                    </div>
                    ...
                    <form method="POST" action="{{ route('login') }}" class="space-y-5">
                        ...
                        <button type="submit"
                            ...
                            <span class="relative inline-flex items-center justify-center gap-2">
                                <span>{{ __('Log in') }}</span>
                                <i class="fa-solid fa-arrow-right text-[12px] opacity-90 group-hover:translate-x-0.5 transition"></i>
                            </span>
                        </button>

                        {{-- small helper line --}}
                        <p class="text-xs text-white/55 text-center pt-1">
                            Tip: You can login using your <span class="text-white/80 font-semibold">email</span> or <span class="text-white/80 font-semibold">username</span>.
                        </p>
                    </form>
                </div>
            </div>
        ...
        <style>
            ...
            @keyframes bgShift{
                0%{ filter: hue-rotate(0deg) saturate(110%); transform: scale(1.02); }
                100%{ filter: hue-rotate(18deg) saturate(130%); transform: scale(1.05); }
            }
            ...
            .p1{ left:10%; bottom:-20px; animation-duration: 11s; transform: scale(1.1); }
            .p2{ left:28%; bottom:-30px; animation-duration: 13s; transform: scale(.9); }
            .p3{ left:55%; bottom:-25px; animation-duration: 10s; transform: scale(1.3); }
            .p4{ left:72%; bottom:-35px; animation-duration: 14s; transform: scale(.85); }
            .p5{ left:88%; bottom:-22px; animation-duration: 12s; transform: scale(1.0); }

            @keyframes floatUp{
                0%{ transform: translateY(0) translateX(0) scale(1); opacity:.0; }
                10%{ opacity:.75; }
                60%{ opacity:.55; }
                100%{ transform: translateY(-880px) translateX(40px) scale(1.35); opacity:0; }
            }
        </style>
